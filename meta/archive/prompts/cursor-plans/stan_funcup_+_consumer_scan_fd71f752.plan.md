...
todos:
  - id: prep-env-docs
    ...
    status: pending
  - id: shared-parse-qr
    content: "product/packages/shared: parseFuncupQrScanPayload + testy jednostkowe"
    status: pending
  - id: expo-camera-scan
...
- **funcup** = dwie aplikacje: `roaster-app` tylko web (`product/apps/web`), `consumer-app` tylko mobile (`product/apps/consumer-mobile`). Wymiana danych przez **kod QR** (model self-hosted: QR generuje palarnia).
...
1. **Monorepo:** upewnij się, że `pnpm install` w korzeniu repo jest wykonane; workspace `@funcup/shared` musi być linkowany do `product/apps/consumer-mobile`.
2. **Supabase + mobile:** zweryfikuj, że `product/apps/consumer-mobile` ma ustawione **`EXPO_PUBLIC_SUPABASE_URL`** i **`EXPO_PUBLIC_SUPABASE_ANON_KEY`** (lub że fallbacki w [`app.config.ts`](product/apps/consumer-mobile/app.config.ts) są świadomie używane tylko do lokalnego dev). **Ten sam projekt Supabase** musi zawierać dane tagów skanowanych z QR wygenerowanego przez roaster-app.
3. **Przepływ referencyjny (do ręcznego testu po implementacji):**
   - Uruchom `product/apps/web` (`pnpm -C product/apps/web dev`), zaloguj palarnię, na **`/tag`** zapisz tag i wygeneruj QR (**Generuj kod QR**).
   - Uruchom consumer (`pnpm -C product/apps/consumer-mobile start` → iOS Simulator / Android / urządzenie).
...
4. **Czytaj przed edycją:** [`product/packages/shared/src/hooks/useCoffeePage.ts`](product/packages/shared/src/hooks/useCoffeePage.ts), [`product/apps/consumer-mobile/app/q/[hash].tsx`](product/apps/consumer-mobile/app/q/[hash].tsx), [`product/apps/consumer-mobile/app/coffee/[id]/index.tsx`](product/apps/consumer-mobile/app/coffee/[id]/index.tsx), [`product/apps/web/app/api/qr/route.ts`](product/apps/web/app/api/qr/route.ts) (format URL w QR: `{origin}/q/{public_hash}`).

### Faza 1 — parsowanie łańcucha z QR (współdzielone + testy)

1. W **`product/packages/shared`** dodaj funkcję (nazwa robocza: `parseFuncupQrScanPayload`), która z **jednego stringa** zwraca **`public_hash`** lub `null`:
   - sam hash (identyfikator jak w `public_hash` w bazie),
   - pełny URL `http(s)://…/q/{hash}`,
   - deep link `funcup://q/{hash}` (zgodnie ze [`scheme` w app.config](product/apps/consumer-mobile/app.config.ts)).
2. **Nie** duplikuj logiki resolvowania batch vs tag — to robi Edge `scan_qr`; parser tylko **wydobywa hash** z tekstu QR.
3. Eksportuj z paczki zgodnie z konwencją [`product/packages/shared/src/index.ts`](product/packages/shared/src/index.ts).
4. Dodaj **testy jednostkowe** (Vitest w repo — użyj istniejącego wzorca w `product/packages/shared`) dla kilku przykładów: sam hash, URL z portem localhost, URL produkcyjny, `funcup://`, negatywne przypadki.

### Faza 2 — ekran skanera (Expo)

1. Zastąp placeholder w [`product/apps/consumer-mobile/app/(tabs)/hub/scan.tsx`](product/apps/consumer-mobile/app/(tabs)/hub/scan.tsx).
2. Użyj **`expo-camera`** (już w [`package.json`](product/apps/consumer-mobile/package.json)) — API zgodne z **Expo SDK 54** / dokumentacją (np. `CameraView`, skan kodów kreskowych/QR).
...
1. W [`product/apps/consumer-mobile/app.config.ts`](product/apps/consumer-mobile/app.config.ts) dodaj plugin **`expo-camera`** w tablicy `plugins`, tak aby na **iOS** pojawił się sensowny **`NSCameraUsageDescription`**, a na **Android** uprawnienie kamery było deklarowane — inaczej release/TestFlight zawiedzie.

### Faza 4 — Coffee Hub (minimalnie)

1. Upewnij się, że CTA z [`hub/index.tsx`](product/apps/consumer-mobile/app/(tabs)/hub/index.tsx) nadal prowadzi do działającego **`hub/scan`**. Refactor stylów hubu do `*.styles.ts` **nie jest** częścią minimalnego Done — tylko jeśli użytkownik poprosi w tej samej sesji.
...
- [ ] Brak regresji: deep link `funcup://q/{hash}` (jeśli skonfigurowany) nadal działa z [`app/q/[hash].tsx`](product/apps/consumer-mobile/app/q/[hash].tsx).
