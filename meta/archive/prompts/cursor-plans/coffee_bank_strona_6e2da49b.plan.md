...
- Kawy z formularza na [`product/apps/web/app/tag/page.tsx`](product/apps/web/app/tag/page.tsx) trafiają do **`roaster_coffee_tags`** (nie do tabeli `coffees` z [`useRoasterCoffees`](product/apps/web/src/hooks/useRoasterCoffees.ts)).
- RLS pozwala zalogowanemu użytkownikowi czytać wiersze, gdzie `roaster_id` należy do jego rekordu w `roasters` ([`0008_roaster_coffee_tags_qr.sql`](product/supabase/migrations/0008_roaster_coffee_tags_qr.sql)).
- **Nazwa kawy w tabeli**: pole **`bean_origin_tradename`** (zgodnie z publicznym podglądem tagu w [`product/apps/web/app/q/[hash]/page.tsx`](product/apps/web/app/q/[hash]/page.tsx) — „Trade name”). Sensowny fallback, jeśli puste: np. fragment `bean_origin_farm` lub „Bez nazwy” + skrót `id`, żeby wiersz był rozróżnialny.
...
- **QR**: ponowne użycie [`product/apps/web/app/api/qr/route.ts`](product/apps/web/app/api/qr/route.ts) — po wyborze wiersza wywołanie `POST /api/qr` z `{ tagId }` i nagłówkiem `Authorization: Bearer <access_token>` (identyczna logika jak [`handleGenerateQr`](product/apps/web/app/tag/page.tsx) w `/tag`). Odpowiedź `{ svg, png, url }` — PNG jako `data:image/png;base64,...`, pobieranie SVG jak [`downloadSvg`](product/apps/web/app/tag/page.tsx) (`qr-${public_hash}.svg`).

## Identyczność kodu QR z pierwszym generowaniem (na `/tag`)

**Ten sam kod ścieżki serwera.** Ponowne wywołanie z Coffee Bank **nie** może używać innej biblioteki ani innych parametrów niż pierwsze generowanie — oba przypadki muszą iść wyłącznie przez [`POST /api/qr`](product/apps/web/app/api/qr/route.ts). W handlerze URL w kodzie to `${origin}/q/${public_hash}`, przy czym `public_hash` jest **niezmienny** dla wiersza (trigger w migracji). `QRCode.toString` / `toBuffer` używają stałych opcji (`margin: 1`, `width: 256`, ten sam typ PNG/SVG) — przy **identycznym łańcuchu URL** wynik graficzny (SVG/PNG) jest deterministyczny.
...
1. **Powłoka**: `pageShell` / `contentInner` wg wzoru z [`tag.styles.ts`](product/apps/web/app/tag/tag.styles.ts) (`authWebShellClasses.page`, `max-w-*`, padding).
...
7. **Prawa kolumna — karta produktu**:
   - Pola z wiersza `roaster_coffee_tags`: m.in. `img_coffee_label`, `roaster_short_name`, pochodzenie, odmiany, obróbka, poziom palenia, `brew_method`, itd. — układ czytelny (sekcje lub lista definicji), spójny wizualnie z kartami z [`tag.styles`](product/apps/web/app/tag/tag.styles.ts) / [`hub-crud.styles`](product/apps/web/app/roaster-hub/hub-crud.styles.ts).
...
## Style (roaster-app = `product/apps/web` — bez klas inline w `page.tsx`)

**Źródło prawdy dla wyglądu strony Coffee Bank** wyłącznie pliki stylów utworzone pod aplikację web palarni, w tym:

- **Nowy moduł** [`product/apps/web/app/coffee-bank/coffee-bank.styles.ts`](product/apps/web/app/coffee-bank/coffee-bank.styles.ts): eksport obiektu `coffeeBankStyles` z **pełnymi literałami klas** Tailwind (JIT skanuje ten plik — jak [`tag.styles.ts`](product/apps/web/app/tag/tag.styles.ts), [`hub-crud.styles.ts`](product/apps/web/app/roaster-hub/hub-crud.styles.ts), [`roaster-profile.styles.ts`](product/apps/web/app/roaster-profile/roaster-profile.styles.ts)).
...
**Zakaz w [`page.tsx`](product/apps/web/app/coffee-bank/page.tsx):**
...
**Współdzielony pakiet:** [`product/packages/shared/src/authWebShellClasses.ts`](product/packages/shared/src/authWebShellClasses.ts) jest importowany **w module stylów** strony (jak na `/tag`), nie jako powód do pisania nowych literałów w `page.tsx`.
...
- Dodać np. [`product/apps/web/src/hooks/useRoasterCoffeeTags.ts`](product/apps/web/src/hooks/useRoasterCoffeeTags.ts) z `useQuery` i kluczem `['roaster-coffee-tags', roasterId]`:
...
- Alternatywa: jeden komponent-rodzic z `useEffect` + `useState` — hook jest czytelniejszy i zgodny z istniejącym [`QueryClientProvider`](product/apps/web/app/providers.tsx).
...
| Plik | Akcja |
|------|--------|
| [`product/apps/web/app/coffee-bank/page.tsx`](product/apps/web/app/coffee-bank/page.tsx) | Pełna implementacja UI + logika wyboru, sortowania, QR |
| `product/apps/web/app/coffee-bank/coffee-bank.styles.ts` | **Nowy** — klasy Tailwind |
| `product/apps/web/src/hooks/useRoasterCoffeeTags.ts` | **Nowy** (zalecane) — lista tagów dla palarni |

**Bez zmian** w [`product/apps/web/app/api/qr/route.ts`](product/apps/web/app/api/qr/route.ts) dla samej funkcji Coffee Bank — ten sam handler gwarantuje ten sam algorytm co przy pierwszym generowaniu; **identyczność łańcucha w QR** wymaga ustawionego kanonicznego [`NEXT_PUBLIC_APP_URL`](product/apps/web/app/api/qr/route.ts) (patrz sekcja wyżej). Opcjonalna późniejsza poprawka do `resolveOrigin` (np. tylko env w prod) — poza minimalnym zakresem strony.
