...
import AnalyticsTrendChart from '@/src/components/analytics/AnalyticsTrendChart';
...
import { useRoasterAnalyticsDashboard, type AnalyticsDashboardFilters } from '@/src/hooks/useRoasterAnalyticsDashboard';
...
function mapSelectedBrewMethods(options: BrewMethodOption[], selectedIds: string[]): ComparisonItem[] {
  return options
...
function mapSelectedTastingNotes(options: TastingNoteOption[], selectedIds: string[]): ComparisonItem[] {
  return options
    .filter((option) => selectedIds.includes(option.id))
    .map((option) => ({
      id: option.id,
...
export function BatchAnalyticsDetail(props: BatchAnalyticsDetailProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setDetailError(null);
      try {
        const nextDetail = await getRoasterBatchPublicationDetail(supabaseBrowser, batchId);
        if (!cancelled) {
  ...
  const sanitizedSuggestedBrewMethodIds = useMemo(
    () => sanitizeSelectedIds(detail?.batch.suggestedBrewMethodIds ?? [], brewMethodOptions),
    [detail, brewMethodOptions]
  );
  const sanitizedSuggestedTastingNoteIds = useMemo(
    () => sanitizeSelectedIds(detail?.batch.suggestedTastingNoteIds ?? [], tastingNoteOptions),
    [detail, tastingNoteOptions]
  );
  const suggestedBrewMethodItems = useMemo(
    () => mapSelectedBrewMethods(brewMethodOptions, sanitizedSuggestedBrewMethodIds),
    [brewMethodOptions, sanitizedSuggestedBrewMethodIds]
  );
  const suggestedTastingNoteItems = useMemo(
    () => mapSelectedTastingNotes(tastingNoteOptions, sanitizedSuggestedTastingNoteIds),
  ...
  const reportHref = useMemo(() => {
    ...
    if (dashboardState.filters.brewMethodId) params.set('brewMethodId', dashboardState.filters.brewMethodId);
    if (dashboardState.filters.startDate) params.set('startDate', dashboardState.filters.startDate);
    if (dashboardState.filters.endDate) params.set('endDate', dashboardState.filters.endDate);
    if (dashboardState.filters.minRating != null) params.set('minRating', String(dashboardState.filters.minRating));
    if (dashboardState.filters.maxRating != null) params.set('maxRating', String(dashboardState.filters.maxRating));
    if (dashboardState.filters.feedbackQuery) params.set('feedbackQuery', dashboardState.filters.feedbackQuery);
  ...
  const filteredBrewMethodComparison = useMemo(
    () => compareSuggestedIdsToObserved(suggestedBrewMethodItems, observedFilteredBrewMethodItems),
    [suggestedBrewMethodItems, observedFilteredBrewMethodItems]
  );
  const filteredTastingNoteComparison = useMemo(
    () => compareSuggestedIdsToObserved(suggestedTastingNoteItems, observedFilteredTastingNoteItems),
  ...
  return (
    <main className={hubCrudStyles.main760}>
      ...
      <p className={`${hubCrudStyles.muted} mb-5 max-w-[820px]`}>
        Review consumer feedback, compare declared Sensory Core targets against actual tastings, and monitor batch performance without entering the batch editing workflow.
      </p>
        ...
        <div className="bg-vs-hero px-6 py-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-vs-hero-contrast/80">{batchMeta(detail)}</p>
              ...
              {!props.reportMode ? (
                <>
                  <Button asChild variant="secondary" size="sm">
                    <Link href={`/roaster-hub/batches/${batchId}`}>Manage batch</Link>
                  </Button>
                  {detail?.qr?.hash ? (
                    <Button asChild variant="secondary" size="sm">
      ...
      <div className="mt-6 space-y-8">
        ...
        {detailError ? <p className={hubCrudStyles.error}>{detailError}</p> : null}
        {taxonomyError ? <p className={hubCrudStyles.error}>{taxonomyError}</p> : null}
        {analytics.error ? (
        ...
        {analytics.isLoading ? (
          ...
          <>
            ...
            <AnalyticsFilterBar
              ...
              onChange={(updater) => dashboardState.setFilters((current) => updater(current))}
            ...
            <div className="grid gap-6 xl:grid-cols-2">
              ...
              <DeclaredTelemetryComparisonCard
                ...
                declaredSensoryAcidity={detail?.batch.declaredSensoryAcidity ?? null}
                declaredSensorySweetness={detail?.batch.declaredSensorySweetness ?? null}
                declaredSensoryBody={detail?.batch.declaredSensoryBody ?? null}
                declaredSensoryBitter={detail?.batch.declaredSensoryBitter ?? null}
                declaredSensoryAftertaste={detail?.batch.declaredSensoryAftertaste ?? null}
