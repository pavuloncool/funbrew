...
export default function AnalyticsFilterBar(props: Props) {
  const { filters, brewMethods, onChange, disabled, reportMode, layout = 'bar' } = props;
  ...
  return (
    <section
      className={cn(
        'rounded-vs-lg border-2 border-vs-border-strong bg-vs-elevated shadow-vs-md',
        sidebarLayout ? 'p-4' : 'p-4 md:p-5'
      )}
    >
      <div
        className={cn(
          'flex items-start justify-between gap-3',
          sidebarLayout ? 'flex-col' : 'flex-wrap'
        )}
      >
        ...
        <div className={cn('flex items-center gap-2', sidebarLayout && 'w-full')}>
          {!reportMode ? (
            <Button
              type="button"
              variant="secondary"
              size="sm"
              className={cn(sidebarLayout && 'w-full')}
              disabled={disabled}
              onClick={() =>
                onChange(() => ({
                  brewMethodId: null,
