...
ALTER TABLE public.roast_batches
  ADD COLUMN IF NOT EXISTS declared_sensory_bitter smallint
    CHECK (declared_sensory_bitter BETWEEN 1 AND 5),
  ADD COLUMN IF NOT EXISTS declared_sensory_aftertaste smallint
    CHECK (declared_sensory_aftertaste BETWEEN 1 AND 5);

ALTER TABLE public.coffee_log_telemetry_core
  ADD COLUMN IF NOT EXISTS sensory_bitter smallint
    CHECK (sensory_bitter BETWEEN 1 AND 5),
  ADD COLUMN IF NOT EXISTS sensory_aftertaste smallint
    CHECK (sensory_aftertaste BETWEEN 1 AND 5);

DROP FUNCTION IF EXISTS public.get_roaster_batch_telemetry_summary(uuid);

CREATE FUNCTION public.get_roaster_batch_telemetry_summary(p_batch_id uuid)
RETURNS TABLE (
  row_scope text,
  brew_method_id uuid,
  total_logs integer,
  ...
  avg_sensory_bitter numeric,
  avg_sensory_aftertaste numeric,
...
AS $$
  ...
  SELECT
    ...
    AVG(j.sensory_bitter)::numeric AS avg_sensory_bitter,
    AVG(j.sensory_aftertaste)::numeric AS avg_sensory_aftertaste,
