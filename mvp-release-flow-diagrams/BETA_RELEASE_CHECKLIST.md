...
Integration source of truth: `mvp-release-flow-diagrams/BETA_INTEGRATION_CONTRACT.md`

## Beta system definition (release hard gate)
- [x] Beta is one system: `apps/web` + `apps/consumer-mobile` + `packages/shared` + one shared Supabase backend.
...
- PASS when:
  ...
  - `apps/web` and `apps/consumer-mobile` point to the same Supabase beta,
...
- Evidence required:
  ...
  - `scripts/mobile-functions-smoke-check.sh` result against beta endpoint.
...
- Status: **IN PROGRESS**
- Evidence baseline: `BETA_SMOKE_RUNBOOK.md` + `scripts/beta-smoke-tests.sh` + `BETA_READINESS_REPORT_2026-05-07.md`.

## Cross-app smoke checks (release-blocking)
- [ ] `bash scripts/beta-smoke-tests.sh` => PASS (required steps + fallback policy respected).
- [ ] `bash scripts/mobile-functions-smoke-check.sh <SUPABASE_FUNCTIONS_BASE_URL>` => PASS (`scan_qr`, `log_tasting` or legacy fallback, `update_coffee_stats`).
