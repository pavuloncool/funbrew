...
- Beta is one integrated system:
  - `apps/web` (roaster + public web entry),
  - `apps/consumer-mobile` (consumer app),
  - `packages/shared` (contracts/services),
...
Primary checklist: `mvp-release-flow-diagrams/BETA_RELEASE_CHECKLIST.md`

## 3) Smoke verification model (must be used for RC)
- Functions readiness:
```bash
bash scripts/mobile-functions-smoke-check.sh https://<project-ref>.supabase.co/functions/v1
```
- Code-level smoke:
```bash
...
Runbook: `mvp-release-flow-diagrams/BETA_SMOKE_RUNBOOK.md`

## 4) Final deploy readiness (cross-app)
- Final deploy checklist is now explicit in:
  - `mvp-release-flow-diagrams/BETA_RELEASE_CHECKLIST.md` (section: `Final deploy checklist`),
...
- Shared write path for tasting:
  - `packages/shared/src/services/tastingService.ts` invokes `log_tasting`,
