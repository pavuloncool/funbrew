...
Scope: `apps/web` + `apps/consumer-mobile` + `packages/shared` + Supabase beta  
Integration reference: `mvp-release-flow-diagrams/BETA_INTEGRATION_CONTRACT.md`

## 1) Current MVP baseline

### A. Role boundaries and entry points
- `apps/web` serves roaster flow and public entry (`/`, `/q/{hash}`).
- `apps/consumer-mobile` serves consumer flow (auth + tabs + scan + coffee + log).
