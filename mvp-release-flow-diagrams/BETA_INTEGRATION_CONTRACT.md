...
- Beta to jeden system dwuwarstwowy:
  ...
  - `apps/consumer-mobile` (consumer flow),
  - wspólne `packages/shared`,
...
- `roaster`:
  - ma dostęp do `apps/web` i ścieżek roasterowych (`/roaster-hub`, setup, analytics, publish batch),
  - nie jest blokowany przez mobile role gate.
- `consumer`:
  - używa `apps/consumer-mobile`,
...
- Canonical shared write path dla nowego wpisu:
  - `packages/shared/services/tastingService.logTasting` -> `log_tasting` (fallback: `coffee/log-tasting`),
  - następnie `packages/shared/services/tastingService.updateCoffeeStats` -> `update_coffee_stats`.
