...
Source contract: `mvp-release-flow-diagrams/BETA_INTEGRATION_CONTRACT.md`

## 1) Scope lock
- Beta release to jeden system: `apps/web` + `apps/consumer-mobile` + shared Supabase beta.
- `apps/web` deploy bez gotowej konfiguracji mobile beta jest niedozwolony.
...
### 2c) Mobile beta build (`apps/consumer-mobile`, EAS env)
...
1. Freeze RC inputs:
- Pin commit SHA for `apps/web`, `apps/consumer-mobile`, `packages/shared`, `supabase/`.
...
- Run smoke/test sequence from `mvp-release-flow-diagrams/BETA_SMOKE_RUNBOOK.md`.
