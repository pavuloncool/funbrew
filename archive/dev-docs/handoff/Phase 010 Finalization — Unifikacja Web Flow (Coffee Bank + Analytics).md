...
#### Implementation Changes (apps/web)
- Routing i IA:
...
- Coffee Bank data flow:
  - W [coffee-bank/page.tsx](/Users/pa/projects/funcup/apps/web/app/coffee-bank/page.tsx) zastąpić `useRoasterCoffeeTags` zapytaniem po roaster-owned `coffees` + ich batchach + `qr_codes`.
...
- Publish/QR/Analytics continuity:
  - W [CoffeeEditor.tsx](/Users/pa/projects/funcup/apps/web/src/components/roaster-hub/CoffeeEditor.tsx) po publish dodać CTA do `Batch details` i `Batch analytics` (nie tylko download SVG).
  - W [roaster-hub/analytics/page.tsx](/Users/pa/projects/funcup/apps/web/app/roaster-hub/analytics/page.tsx) zastąpić statyczny komunikat listą batchy roastera z quick stats + empty state.
...
- Integration/runtime:
  - `apps/web test`, lokalne `apps/web dev`, smoke: publish -> QR -> batch analytics.
- Regression:
