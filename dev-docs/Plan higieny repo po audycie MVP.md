...
- Jeśli coś ma być czytelne po miesiącu bez `git log`, trafia do `archive/` w repo.
...
- legacy app files:
  - `old-files/`
  - `unused project files/`
  - scaffolding typu `apps/frontend`, jeśli nie jest już aktywny
- stare prompty:
  - `.cursor/plans/*`
...
## Założenia
- `archive/` jest preferowanym celem dla rzeczy historycznie istotnych.
